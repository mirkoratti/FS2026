...
% --------------------------------------- COLOR DEFINITIONS ---------------------------------------
\definecolor{sectioncolor}{HTML}{6B3881}
\definecolor{subsectioncolor}{HTML}{E3D1EB}
\definecolor{sectextcol}{HTML}{FFFFFF}
...
% --------------------------------- ARM ASSEMBLY STYLE ---------------------------------
\lstdefinestyle{armasmstyle}{
    language={}, % Rimuove la dipendenza da driver esterni
    morecomment=[l]{;},
    morecomment=[l]{@},
    morestring=[b]",
    % === Istruzioni Thumb-2 e Flag-setting ===
    morekeywords={%
        ADDS, ADCS, SUBS, SBCS, LSLS, LSRS, ASRS, RORS, 
        BICS, STRH, LDRH, STRB, LDRB, LDRSH, LDRSB, 
        MOVS, MVNS, ANDS, ORRS, EORS, MRS, MSR,
        LDR, STR, LDM, STM, PUSH, POP, BL, BX, B, MOV, CMN, BLE, CMP, BGT, BGE, SXTH
    },
    % === Registri (Gruppo 2) ===
    ...
    morekeywords={[2]%
        ...
        R0, R1, R2, R3, R4, R5, R6, R7, R8, R9, R10, R11, R12, R13, R14, R15,
        sp, lr, pc, SP, LR, PC, psr, apsr, PSR, APSR
    },
    % === Direttive (Gruppo 3) ===
    keywordstyle={[3]\color{blue!80!black}},
    morekeywords={[3]%
        .text, .data, .bss, .global, .extern, .ascii, .asciz, .word, .byte, .align, .end, .section
    },
    alsoletter={.}, 
    % labels
    keywordstyle={[4]\color{orange!20!black}},
    morekeywords={[4]%
        else
    },
} %
