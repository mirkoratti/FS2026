...
\begin{itemize}
    ...
    \item \textbf{Operatore noexcept(expr):} \cor{(valutato a compile time)} Restituisce \texttt{true} se l'espressione è specificata come \texttt{noexcept}.
\end{itemize}
...
\subsection{array}
\begin{itemize}
\item \textbf{Definizione:} \texttt{std::array<T, size>} è un contenitore a dimensione 
fissa implementato come classe template.
\item \textbf{Dimensione:} \texttt{std::array} conosce la propria dimensione tramite la 
funzione membro \texttt{.size()}.
\item \textbf{Accesso ai dati:} classica notazione a parentesi quadre \texttt{[]}.
\item \textbf{Sicurezza:} \texttt{.at()} è accesso sicuro con controllo dei limiti, eccezione \lstinline|std::out_of_range| in caso di accesso non valido.
\item Nei parametri delle funzioni passabili come \textbf{\color{red}referenza, sempre da fare} (perchè sono una classe)
\end{itemize}
...
\begin{itemize}
\item \textbf{Struttura:} Dimensione esterna: righe, interna: colonne.
\item \cor{Richiede una doppia coppia di parentesi graffe per l'inizializzazione} 
(perchè \texttt{std:array} è una classe)
\item \textbf{Accesso:} doppia index notation: \texttt{matrix[riga][colonna]}.
\end{itemize}
