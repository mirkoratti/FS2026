...
Eine Exception wird bei Fehlerauftritt \texttt{geworfen} (throw), und zur Behandlung \texttt{gefangen} (catch).


\lstinputlisting{code/04_Memorymanagement/exceptionGenerellSyntax.cpp}
...
\lstinputlisting{code/04_Memorymanagement/throwCatchV2.cpp}
...
Wenn der letzte Catch block mit \texttt{...} aufgerufen wird, werden alle Typen Exceptions gehandelt. 

\lstinputlisting{code/04_Memorymanagement/catchcatcher.cpp}
...
Falls dieses Verhalten angepasst werden soll, kann man das mit \texttt{set\_terminate(std::terminate\_handler f)}.
...
Um zu signalisieren, ob eine Funktion eine Exception auslösen kann oder nicht, kann der specifier \texttt{\textbf{noexept}} verwendet werden. 
...
\begin{ctabular}{ccc} \lstinline|std::logic_error| & \textbf{std::out\_of\_range} & \textbf{std::overflow\_error} \end{ctabular}

Für Details siehe :\url{https://en.cppreference.com/w/cpp/header/stdexcept}
