...
\subsection{Picard-Lindelöf (Eindeutigkeit der Lösung)}

Schema zur Beurteilung der \textbf{Lösbarkeit} einer DGL an einem Punkt $(x_0, y_0)$:

\medskip
\textbf{Schritt 1: Stetigkeit (Existenz)}
\begin{itemize}
    \item[(1)] $f(x, y, y', \, ... \, y^{(n-1)} )$ lokal bei $x_0 ; y_0$ stetig.
    \item[] Stetigkeit rimane con operazioni elementari: $+, -, \cdot, :, \text{subst (\crd{Att}. }\mathbb{D}) (\sqrt{f}, \ln(f))$\\
    \cor{Lineare oder elementare Funktionen sind fast immer stetig.}
\end{itemize}

\textbf{Schritt 2: Beschränktheit (Eindeutigkeit)}
\begin{itemize}
    \item[(2)] Alle partiellen Ableitungen bilden: $f_y = \frac{\partial f}{\partial y}$ ; $f_{y'} = \frac{\partial f}{\partial y'}$ ; ...
    \begin{itemize}
        \item Ist das Ergebnis \textbf{konstant} \textrightarrow\ automatisch lokal beschränkt.
        \item Ist es eine \textbf{Formel} \textrightarrow\ $x_0, y_0$ einsetzen. 
        Das Ergebnis muss $\in \mathbb{R}$
    \end{itemize}
\end{itemize}
...
\textbf{Fazit:}
\begin{itemize}
    \item \textbf{Wenn} (1) erfüllt \textbf{und} (2) lokal bei $x_0 ; y_0$ beschränkt: \\
    \textrightarrow\ Die DGL ist \textbf{lokal eindeutig lösbar} (inkl. Anfangsbedingungen).
    \item \textbf{Wenn} (1) erfüllt, \textbf{aber} (2) \myul{nicht} erfüllt (z.B. $\frac{1}{0}$ in der Ableitung): \\
    \textrightarrow\ Eindeutigkeit ist nicht garantiert. \textbf{Singularitäts-Test durchführen!}
\end{itemize}

\medskip
\subsubsection{Singularitäts-Test (Ablauf bei Polstelle in $f_y$):}
\begin{enumerate}
    \item \textbf{Polstelle finden:} Nenner der partiellen Ableitung $= 0$ setzen.
    \item \textbf{Auflösen:} Diese Gleichung nach $y$ auflösen (das liefert die singuläre Kandidaten-Lösung).
    \item \textbf{Verifizieren:} Diese Funktion $y$ in die \textit{ursprüngliche} DGL einsetzen und kontrollieren, ob die Gleichung erfüllt ist.
\end{enumerate}
...
\begin{tabular}{ll}
    ...
    (6) & \crd{\textbf{Singularitätstest} um weitere Lösungen zu finden (vedi \ref{chap:sing_erg})}\\
\end{tabular}
...
$$ y' = f(x \, ; \, y) = f( \underbrace{ax + by +c}_{\substack{z}}) \qquad \qquad y(x_0) = y_0 $$\vspace{-1em}

Separiert DGL: \quad $\boxed{z' = a + b \cdot f(z)}$ \qquad \qquad $z(x_0) = z_0 = a  \,x_0 + b \, y_0 + c$ 

\smallskip
\begin{itemize}
    \item weiter in Abschnitt \ref{Trennung der Variablen (separiert)} 
    mit $\tilde{f}(x) = 1$ (Non ci sono piu fattori dipendenti da $x$)
    \item Substitution $y = z$
\end{itemize}

\subsubsection{Gleichgradigkeit}\vspace{-0.2cm}

$$ y' = f(x \, ; \, y) = f \underbrace{\left(\frac{y}{x}\right)}_z \qquad \qquad y(x_0) = y_0 ; (x ; x_0 \neq 0) $$\vspace{-0.5em}

Separiert DGL: \quad $\boxed{z' = \frac{1}{x} (f(z) - z)}$ \qquad \qquad $z(x_0) = z_0 = \frac{y_0}{x_0}$ 

\begin{itemize}
    \item weiter in Abschnitt \ref{Trennung der Variablen (separiert)} mit $\tilde{f}(x) = \frac{1}{x}$, Substitution $y = z$ 
\end{itemize}
...
\begin{itemize}
    ...
    \item Ableitung (Steigung) $y'$ steht im Kehrwert $\frac{1}{y'} \, \longrightarrow \, \tilde{x}' = \frac{1}{\tilde{y}'}$
\end{itemize}


\subsection[Lineare DGL n=1]{Lineare DGL $\bm{n=1}$}\vspace{-0.3cm}

$$ 1 \, y' +  f(x) \, y = g(x) \qquad g(x) \text{ ist Störterm} \qquad \text{Anfangswert: } y(x_0) = y_0 $$
$$ \text{\textbf{Lösungsmenge:}} \quad \boxed{\mathbb{L} = \mathbb{L}_H + y_p} \quad (\bm{y_p} \text{ \textbf{ist stationärer Anteil}}) $$
...
\[y = y_0 \cdot \e^{- \int \limits_{x_0}^{x} \cgn{f(\tilde{x})} \, d\tilde{x}} \qquad 
(y_0 \in \mathbb{R}) \text{ \textrightarrow\ } \mathbb{L}_H\]


\subsubsection{Partikulär / Inhomogen \textrightarrow\ Störterm $\bm{g(x) \neq 0}$}\vspace{-0.2cm}

\[y_p = k(x) \cdot  \e^{- \int \cgn{f(x)} \, \diff x} \quad \text{mit} \quad k(x) 
= \int \cbl{g(x)} \cdot \e^{\, \int \cgn{f(x)} \, \diff x} \, \diff x\]
...
\begin{minipage}{0.3\columnwidth}
    \textbf{Risoluzione:}
    \begin{enumerate}
        \item Equaz. con parametro var. $c$
        \item Risolvere per $c$
        \item Ableitung equazione pto.1
        \item Sostiture $c$
        \item Risovlere DGL
    \end{enumerate}
\end{minipage}
...
\begin{tabular}{ll}
    ...
    (2) & DGL $y'$ in neue DGL \cbl{$\tilde{y}'$} transformieren                \\
        ...
        & $y' = f(x \, ; \, y)$ \quad \textrightarrow\ \quad \cbl{$\tilde{y}' = - \dfrac{1}{f(x \, ; \, \tilde{y})}$} \qquad \textrightarrow\ DGL \cbl{$\tilde{y}'$} lösen!
\end{tabular}
...
$$ \text{\colorbox{yellow!50}{char. Gleichung:}} \quad \underbrace{1 \, \lambda^2 + a_1 \, \lambda + a_0}_{\text{char. Polynom}} = 0 \qquad
\lambda_{1,2} = - \frac{a_1}{2} \pm \sqrt{\vphantom{\left(\frac{a_1}{2}\right)^2} 
\smash[b]{\underbrace{\left(\frac{a_1}{2}\right)^2 - a_0}_D}} \vphantom{\underbrace{\left(\frac{a_1}{2}\right)^2 - a_0}_D} $$

\setlength{\tabcolsep}{4pt}
\begin{tabular}{lll}
    $D > 0$ & $\lambda_{1,2} \in \mathbb{R}$ ; $\lambda_1 \neq \lambda_2$ & (starke Dämpfung)                                                                      \\ \medskip
            & $\mathbb{L}_H = A \, \e^{\lambda_1 \, x} + B \, \e^{\lambda_2 \, x}$ & ($A, B \in \mathbb{R}$)                                                        \\ 
    $D < 0$ & $\lambda_{1,2} \notin \mathbb{R}$ ; $\lambda_{1,2} = - \frac{a_1}{2} \pm j \cdot \underbrace{\sqrt{\vert D \vert}}_{\substack{\alpha}}$ & (Schwingfall)    \\ \medskip
            & $\mathbb{L}_H = A \, \e^{- \frac{a_1}{2} x} \cdot \cos(\alpha \, x) + B \, \e^{- \frac{a_1}{2} x} \cdot \sin(\alpha \, x)$ & ($A, B \in \mathbb{R}$)  \\
    $D = 0$ & $\lambda_1 = \lambda_2 = - \frac{a_1}{2} \in \mathbb{R}$ & (Aperiodischer Grenzfall)                                                                 \\
...
\begin{tabular}{ll}
    ...
    $\cbl{q_n(x)}$  & Lösungs-Polynom vom gleichen Grad $n$ \quad (z.B. $a x^2 + b x + c$ für $n = 2$)  \\  \medskip
                    & Koeffizienten bestimmen durch einsetzen in DGL ($y_p'' \rightarrow y'', y_p' \rightarrow y', \dots$)                   \\  \medskip
    $r_n$, $s_n$    & Amplituden \textrightarrow\ $y_p$ in DGL einsetzen (come $y$)                              \\  
...
\begin{tabular}{ll}
    ...
    b)  & wenn $b$ \textbf{einfache} Lösung der char. Gleichung ist (\cor{Resonanz}) \\  \medskip
        & \textrightarrow\ $y_p = \e^{b x} x \, \cbl{q_n(x)}$       \\
    c)  & wenn $b$ \textbf{doppelte} Lösung der char. Gleichung ist ($D=0, \, \lambda_1 = \lambda_2$)\\
        & (\cor{Resonanz ordnung 2}) \\
...
\begin{tabular}{ll}
    a)  & wenn $c + \jimg b$ \textbf{keine} Lösung der \colorbox{yellow!50}{char. Gleichung} ist                                       \\  \medskip
        & \textrightarrow\ $y_p = \e^{c x} \big( r_n(x) \, \cos(bx) + s_n(x) \, \sin(bx) \big)$         \\
    b)  & wenn $c + \jimg b$ Lösung der \colorbox{yellow!50}{char. Gleichung} ist                                             \\
        & \textrightarrow\ $y_p = \e^{c x} \, x \, \big( r_n(x) \, \cos(bx) + s_n(x) \, \sin(bx) \big)$
\end{tabular}\medskip

(Verificare se $c$ e $b$ sono ugualli alla sol. della gleichung)
...
$$ \text{char. Gleichung:} \quad 1 \, \lambda^n + a_{n-1} \, \lambda^{n-1} + \ldots + a_2 \, \lambda^2 + a_1 \, \lambda + a_0 = 0 $$ 

\textit{$r, k$: numero di ripetizioni $\lambda$}
...
Vorteile: wie \ref{chap:inhomogen_n2}
...
\begin{tabular}{ll}
    ...
    b)  & wenn $\alpha$ \textbf{$\bm{r}$-fache} Lösung der char. Gleichung ist (\cor{Resonanz ordnung $r$}) \\
...
\begin{tabular}{ll}
    ...
    b)  & wenn $\alpha + \jimg \beta$ \textbf{$\bm{r}$-fache} Lösung der char. Gleichung ist (\cor{Resonanz ordnung $r$})                      \\
...
Per ottenere le anfangbedingungen riposetto ad una sola variabile:\vspace{-1em}

\begin{align*}
    \text{\RomanNumeralCaps{1}}: \qquad & (x(x_0), \dot{x}(x_0)) \quad \text{mit} \quad \dot{x}(x_0) = ax(x_0) + by(x_0) + e(x_0)\\
    \text{\RomanNumeralCaps{2}}: \qquad & (y(x_0), \dot{y}(x_0)) \quad \text{mit} \quad \dot{y}(x_0) = cx(x_0) + dy(x_0) + f(x_0)\\
