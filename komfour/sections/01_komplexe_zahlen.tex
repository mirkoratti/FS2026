...
\crd{Wichtige Eigenschaften von komplexen \textbf{und} reellen Polynomen:}

\smallskip

\begin{enumerate}[itemsep=0.2cm]
    \item Le soluzioni complesse possono essere totalmente asimmetriche e non presentarsi in coppia
...
\end{ctabular}
\begin{center}
    \textit{Dividendo un $p(z)$ con una NS (Nullstelle), il resto è \textbf{SEMPRE = 0}}
\end{center}

\subsubsection{Spezialfall: Polynome mit Koeffizienten $\bm{\in \mathbb{R}}$}

\crd{Wichtige Eigenschaften von \textbf{reellen}  Polynomen:}
...
\subsection{Sovrapposizione di oscillazioni sinusoidali ($\mathbf{\omega}$ = const)}

\vspace{-0.2cm}
\[ \text{Oscillazione reale:} \quad x(t) = A \cdot \sin(\omega t + \varphi) \quad \text{con } \omega = \frac{2 \pi}{T} \]
\vspace{-1.5em}

\[ \sin(\omega t + \varphi) = \IM\left\{ \e^{\jimg (\omega t + \varphi)} \right\} \quad \text{e} \quad \cos(\omega t + \varphi) = \RE\left\{ \e^{\jimg (\omega t + \varphi)} \right\} \]
\vspace{-1.5em}
...
\begin{tabular}{lll}
    Ampiezza complessa  & $\underline{A} = A \cdot \e^{\jimg \varphi}$ & \crd{(\textbf{per} $\bm{t=0}$)} \\
    Oscillazione complessa & $z(t) = A \cdot \e^{\jimg (\omega t + \varphi)} = \underline{A} \cdot \e^{\jimg \omega t}$ & (dipendente dal tempo)
\end{tabular}

\smallbreak

\para{Sovrapposizione di oscillazioni isofrequenziali ($z(t) = z_1(t) + z_2(t)$)}
\begin{itemize}
    \item \textbf{Principio:} Equivale alla somma grafica dei vettori rotanti nel piano complesso.
    \item \textbf{Prerequisito:} Uniformare i segnali tutti in $\sin$ o tutti in $\cos$ (usando \ref{chap:trigo-beziehungen}).
    \item \textbf{Metodo:} $\underline{A}_{res} = \underline{A}_1 + \underline{A}_2$. Convertire da polare ($A\e^{\jimg\varphi}$) a cartesiano ($x + \jimg y$), sommare componente per componente e riconvertire in polare per trovare l'ampiezza $A_{res}$ e la fase $\varphi_{res}$ finali.
\end{itemize}

\smallskip

\textbf{Esempio pratico di calcolo:}
\[ \underline{A}_1 = 2\e^{\jimg 0} = 2 \quad \text{e} \quad \underline{A}_2 = 2\e^{\jimg \frac{\pi}{2}} = 2\jimg \quad \longrightarrow \quad \underline{A}_{res} = 2 + 2\jimg = 2\sqrt{2}\cdot \e^{\jimg \frac{\pi}{4}} \]\vspace{-1em}