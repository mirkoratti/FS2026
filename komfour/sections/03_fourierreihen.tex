...
\multirow{2}{*}{$f(t)$ ungerade} & $\displaystyle a_n = 0$ & $\displaystyle b_n = \frac{4}{T} \int_{0}^{\frac{T}{2}} f(t) \sin(n \omega_f t) dt$ \\
                                 & \color{blue}{$\text{Re}(c_k) = 0$} \qquad \color{blue}{$c_0 = 0$} & \color{blue}{$c_n = -j \frac{b_n}{2}$ (imaginär)} \\ \hline
\end{tabular}
\renewcommand{\arraystretch}{1}

\subsubsection{Linearität}
\begin{center}
    \crd{$f(t) \text{ und } g(t)$ müssen das gleiche Periode $T$ haben.}
\end{center}
...
$$ a_n^{(h)} = r \cdot a_n^{(f)} + s \cdot a_n^{(g)} \qquad b_n^{(h)} = r \cdot b_n^{(f)} + s \cdot b_n^{(g)} \qquad \cbl{c_k^{(h)} = r \cdot c_k^{(f)} + s \cdot c_k^{(g)}}$$

\subsubsection{Zeitstreckung / -stauchung / -spiegelung ("Ähnlichkeit")}

$g(t) = f(r \cdot t)$ mit $0 \neq r \in \mathbb{R}$ \qquad \crd{$\omega_g = \frac{2 \pi}{T_g} = \abs{r} \cdot \omega_f$ in Basisfunktionen! ($\sin / \cos$)}

\[ a_n^{(g)} = a_n^{(f)} \qquad \qquad b_n^{(g)} = \sgn(r) \cdot b_n^{(f)} 
\qquad \qquad \cbl{c_k^{(g)} = c_{k\cdot \sgn(r)}^{(f)}}\]

\subsubsection{Zeitverschiebung}
$g(t) = f(t + t_0)$ \qquad \qquad $A_n$ rimane invariato \textrightarrow $A_n^{(g)} = A_n^{(f)}$

\smallskip

\renewcommand{\arraystretch}{1.5}
\begin{tabular}{ll}
    $a_n^{(g)} = \cos(n \, \omega_f \, t_0) \cdot a_n^{(f)} + \sin(n \, \omega_f \, t_0) \cdot b_n^{(f)}$    &\qquad $n = 0, \, 1, \, 2, \, \ldots$ \\ 
    $b_n^{(g)} = -\sin(n \, \omega_f \, t_0) \cdot a_n^{(f)} + \cos(n \, \omega_f \, t_0) \cdot b_n^{(f)}$   &\qquad $n = 1, \, 2, \, 3, \, \ldots$ \\  
    $\cbl{c_k^{(g)} = \e^{\jimg \, k \, \omega_f \, t_0} \cdot c_k^{(f)}}$                                   &\qquad $\cbl{k \in \mathbb{Z}}$
\end{tabular}
\renewcommand{\arraystretch}{1}
...
$$ \left\| f - g \right\| = \sqrt{\frac{2}{T} \int \limits_0^T  [f(t) - g(t)]^2 \, \diff t } $$ 
...
$$ \frac{a_0^2}{2} + \sum \limits_{n=1}^\infty \big( a_n^2 + b_n^2 \big) = \frac{2}{T} \int \limits_0^T [f(t)]^2 \, \diff t = \left\| f \right\|^2 $$
...
$$ \lim \limits_{m \to \infty} \left\|\sum\limits _{n=1}^m \big[ a_n \cdot \cos(n \, 
\omega_f \, t) + b_n \cdot \sin(n \, \omega_f \, t) \big] - f(t) \right\| = 0 $$
$$ \cbl{\lim \limits_{m \to 0} \left\| \sum\limits _{k= -m}^m \: \big( c_k \, \e^{ \jimg \, k \, \omega_f \, t} \big) - f(t) \right\| = 0} $$


\subsubsection{Konvergenz der Fourierkoeffizienten}

Die Fourierkoeffizienten bilden Nullfolgen (Conseguenza: convergenza)
...
\subsubsection{Punktweise Konvergenz (Satz von Dirichlet)}\label{chapt:punktweise_konvergenz}
...
\begin{itemize}
    \item Attenzione: se \crd{Q \textrightarrow $f(t)$ si trova in una \textbf{sprungstelle}}, usare punto medio (\ref{chapt:punktweise_konvergenz})
    \item RICORDARSI, in $\FR()$ il primo coefficente è $\dfrac{a_0}{\crd{\mathbf{2}}}$ 
\end{itemize}
