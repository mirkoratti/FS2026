...
\subsubsection{Gültigkeit des Modells (Reales vs. Ideales Gas)}
Ein reales Gas (z.B. Luft) verhält sich wie ein ideales Gas, wenn es weit über seinem Verflüssigungspunkt liegt (intermolekulare Kräfte vernachlässigbar).
\begin{itemize}
    \item $T \gg T_{crit} \implies$ Gas ist \textbf{ideal} (z.B. Luft bei Raumtemperatur: $293\,\text{K} \gg 133\,\text{K}$).
    \item Hohe Temperaturen und geringe Drücke begünstigen das ideale Verhalten.
\end{itemize}
...
\begin{tabular}{c l c}
	...
	$T_x$ & \cor{\textbf{Absolut-}Temperatur}			& $[T] = \kelvin$
\end{tabular}
...
\begin{tabular}{c l c}
	...
	$T$ 	& \textbf{Absolut-}Temperatur 														& $[T] = \kelvin$
\end{tabular}
\renewcommand{\arraystretch}{1}\smallskip

\cor{Se calcolo per umidità non usare $\dot{V}$ ma sempre $V$ e trovare poi $\dot{m}$ in \textbf{secondo momento}}
