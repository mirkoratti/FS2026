...
\begin{minipage}[b]{0.49\columnwidth}
    \begin{itemize}
        \item Area sfera: $4\pi r^2$
        \item Volume sfera: $\frac{4}{3} \pi r^3$
    \end{itemize}
\end{minipage}
...
Una pompa di calore assorbe lavoro elettrico ($P_{el}$) per trasferire calore da una sorgente fredda (terreno/aria) a un ambiente caldo.

\paragraph{Principio di Conservazione dell'Energia}
\begin{equation*}
P_H = P_{el} + P_K \implies P_{el} = P_H - P_K
\end{equation*}

\paragraph{Coefficiente di Prestazione reale (COP / $\eta_{WP}$)}
Efficienza reale della macchina (rapporto tra calore utile ottenuto ed energia elettrica spesa):
\begin{equation*}
\eta_{WP} = \frac{P_H}{P_{el}} = \frac{P_H}{P_H - P_K}
\end{equation*}

\begin{itemize}
    \item $P_H$: Potenza termica utile erogata (riscaldamento ambiente) [W]
    \item $P_{el}$: Potenza elettrica assorbita dal compressore [W]
    \item $P_K$: Potenza frigorifera estratta dalla sorgente fredda [W] $\rightarrow P_K = \dot{V}_K \cdot \rho \cdot c \cdot \Delta T_K$
    \item $c$: Capacità termica specifica del fluido [$\text{J}/(\text{kg}\cdot\text{K})$]
\end{itemize}
...
\subsection{Doppelwägung-Methode (Aria vs. Liquido)}
Quando viene fornita la forza risultante sia in aria ($F_{\text{res, Luft}}$) sia in un liquido ($F_{\text{res, Flüssigkeit}}$), bisogna considerare che il corpo subisce la spinta idrostatica dell'aria anche durante la prima pesata.


Uguagliando le espressioni delle forze risultanti nelle due condizioni, si ricava la 
formula diretta per il volume:
\[
V_K = \frac{F_{\text{res, Luft}} - F_{\text{res, Flüssigkeit}}}{g \cdot (\rho_{\text{Flüssigkeit}} - \rho_{\text{Luft}})}
\]

Una volta ottenuto il volume $V_K$, la forza peso esatta (nel vuoto) si calcola riaggiungendo la spinta dell'aria alla misura effettuata in aria:
\[
F_G = F_{\text{res, Luft}} + \rho_{\text{Luft}} \cdot V_K \cdot g
\]
